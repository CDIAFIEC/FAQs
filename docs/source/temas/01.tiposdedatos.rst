...
1. Implemente un TDA `Libro` que tenga las propiedades `titulo`, `autor`, y `numeroPaginas`. Defina los métodos para leer una página, obtener información del libro y cambiar el autor.
2. Implemente el TDA `Biblioteca` que contenga una colección de libros. Defina los métodos para agregar un libro, eliminar un libro y buscar un libro por título.

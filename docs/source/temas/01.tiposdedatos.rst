...
1. Diseñe el TDA `Fracción` con las propiedades y los comportamientos necesarios, considerando: 

   a) Las para `numerador` y `denominador`, 
   b) Los comportamientos `sumar`, `restar`, `multiplicar` y `dividir` fracciones. 
   c) Y, las restricción que el denominador no sea cero y simplifique la fracción al máximo.

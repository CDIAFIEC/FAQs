...
1. Implemente la clase `Pareja<T1, T2>` que permita almacenar un par de elementos de tipos diferentes. 

   (i) Añada el método `equals()` para comparar dos objetos de tipo `Pareja<T1, T2>` basándose en sus elementos.
   (ii) Implemente un método `toString()` que retorne una representación en cadena de la pareja, mostrando ambos elementos.

2. Cree un programa de prueba que instancie un objeto de la clase `Pareja` y muestre sus elementos.
